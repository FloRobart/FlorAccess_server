...
export async function updateUser(updateUser: UpdateUser, userSafe: UserSafe): Promise<User> {
    try {
        ...
        values.push(userSafe.id, userSafe.email, userSafe.pseudo, userSafe.last_logout_at.toString(), userSafe.created_at.toString());
        ...
        query += setClauses.join(', ') + " WHERE id = $1 AND is_connected = true AND email = $2 AND pseudo = $3 AND date_trunc('second', last_logout_at) = date_trunc('second', $4::timestamp) AND date_trunc('second', created_at) = date_trunc('second', $5::timestamp) RETURNING *";
...
export async function deleteUser(userSafe: UserSafe): Promise<User> {
    try {
        let query = "DELETE FROM users WHERE id = $1 AND is_connected = true AND email = $2 AND pseudo = $3 AND date_trunc('second', last_logout_at) = date_trunc('second', $4::timestamp) AND date_trunc('second', created_at) = date_trunc('second', $5::timestamp) RETURNING *";
        let values = [userSafe.id, userSafe.email, userSafe.pseudo, userSafe.last_logout_at.toString(), userSafe.created_at.toString()];
...
export async function logoutUser(userSafe: UserSafe): Promise<User> {
    try {
        let query = "UPDATE users SET is_connected = false, last_logout_at = NOW() WHERE id = $1 AND email = $2 AND pseudo = $3 AND date_trunc('second', last_logout_at) = date_trunc('second', $4::timestamp) AND date_trunc('second', created_at) = date_trunc('second', $5::timestamp) RETURNING *";
        let values = [userSafe.id, userSafe.email, userSafe.pseudo, userSafe.last_logout_at.toString(), userSafe.created_at.toString()];
