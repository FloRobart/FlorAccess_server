...
export async function userLoginRequest(user: User, tokenHash: string, codeHash: string): Promise<void> {
    try {
        const query = "UPDATE users SET token_hash = $6, secret_hash = $7 WHERE id = $1 AND email = $2 AND pseudo = $3 AND date_trunc('second', last_logout_at) = date_trunc('second', $4::timestamp) AND date_trunc('second', created_at) = date_trunc('second', $5::timestamp) RETURNING *";
        const values = [user.id, user.email, user.pseudo, user.last_logout_at.toISOString(), user.created_at.toISOString(), tokenHash, codeHash];
