...
@section('content')
<!-- Titre de la page -->
@include('components.page-title', ['title' => 'Que voulez-vous faire ?'])
    ...
    @foreach ($tools as $tool)
        <div id="position_{{ $tool->position }}" class="rowCenterContainer gap-x-4">
            ...
            <div class="action smallRowCenterContainer px-1 min-[460px]:px-2 min-[500px]:px-4 py-2 hidden">
                <!-- Modifier -->
                <button onclick="editTool('{{ $tool->name }}', '{{ $tool->link }}', '{{ $tool->id }}')" class="smallRowCenterContainer w-fit smallTextReverse font-bold bgBleuLogo hover:bgBleuFonce focus:normalScale rounded-lg min-[500px]:rounded-xl py-1 px-1 min-[500px]:px-2">
